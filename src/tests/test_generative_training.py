...
def test_train_vae_calls_train_and_validate(setup_model):
    ...
    with patch("src.generative.training.train") as mock_train, patch(
        "src.generative.training.validate"
    ) as mock_validate, patch("src.generative.training.save_checkpoint") as mock_save:
        train_vae(
            n_epochs=n_epochs,
            model=model,
            optimizer=optimizer,
