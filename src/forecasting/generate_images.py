...
def generate(model, test_data, background_data, obs_data, start, num_inputs, device):
    ...
    def generate_and_plot_test(data, start, num_inputs, title_prefix, row):
        ...
        target_image = torch.tensor(target_image, dtype=torch.float32).unsqueeze(0).to(device)
    ...
    background_output = generate_and_plot_obs_background(background_data, num_inputs, 'Background', 1)
